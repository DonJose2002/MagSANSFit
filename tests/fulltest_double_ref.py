...
for i in range(3, jointstart.size):
    if i != pos_array_A_2 and i != pos_array_mu_1 and i != pos_array_mu_2:
        current_bound = [max(jointstart[i]-joint_searchwidth[i],jointstart_mag[i]-joint_searchwidth[i]),min(jointstart[i]+joint_searchwidth[i],jointstart_mag[i]+joint_searchwidth[i])]
    else:
        current_bound = [jointstart[i]-joint_searchwidth[i],jointstart[i]+joint_searchwidth[i]]
    
    bounds_jointfit.append(current_bound)

#add mag specific bounds4
bounds_mag_joint = [[max(bounds_box_np[0][0],jointstart_mag[0]-joint_searchwidth[0]),min(bounds_box_np[0][1],jointstart_mag[0]+joint_searchwidth[0])],
...
if distribution_type == "Double":
    ...
    if model_2 == "core-shell":
        bounds_mag_joint.append([max(bounds_box_np[pos_array_mu_2][0],jointstart_mag[pos_array_mu_2]-joint_searchwidth[pos_array_mu_2]),min(bounds_box_np[pos_array_mu_2][1],jointstart_mag[pos_array_mu_2]+joint_searchwidth[pos_array_mu_2])])
else:
    if model_1 == "core-shell":
        bounds_mag_joint.append([max(bounds_box_np[pos_array_mu_1][0],jointstart_mag[pos_array_mu_1]-joint_searchwidth[pos_array_mu_1]),min(bounds_box_np[pos_array_mu_1][1],jointstart_mag[pos_array_mu_1]+joint_searchwidth[pos_array_mu_1])])
