...
from core.models.loss_functions import loss_chi2,noisy_loss_log_chi2
from core.utils.helper_functions import reward_variance
import warnings

def log_chi2_final_intensity_spheroid_BO_single(theta,distribution,formfactor,volume,list_q,experiment,uncertainty):
    ...
    if callable(distribution): # check if distribution is mono
        if num_parameters == 5: #sphere case
            for t in theta_np:
                ...
                chi2 = noisy_loss_log_chi2(experiment,uncertainty,model_prediction,discrepancy)
...
"""
def log_chi2_nano_intensity_spheroid_BO_single(theta,distribution,formfactor,volume,list_q,experiment,uncertainty):
    theta_np = theta.detach().cpu().numpy()
    num_parameters = theta_np.shape[1]-1 #record number of model parameters
    num_evaluations= list_q.size
    results_chi2 = []
    ...
    return torch.tensor(results_chi2, dtype=torch.double)
def log_chi2_final_intensity_spheroid_BO_double(theta, distribution_1, distribution_2, formfactor_1, formfactor_2 ,volume_1, volume_2,list_q,experiment,uncertainty):
    theta_np = theta.detach().cpu().numpy()
...
def log_chi2_nano_intensity_spheroid_BO_double(theta, distribution_1, distribution_2, formfactor_1, formfactor_2 ,volume_1, volume_2,list_q,experiment,uncertainty):
    theta_np = theta.detach().cpu().numpy()
    num_parameters = theta_np.shape[1]-1 #record number of columns
    num_evaluations= list_q.size
    results_chi2 = []
