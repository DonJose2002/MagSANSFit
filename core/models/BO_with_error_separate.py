...
from core.models.loss_functions import loss_chi2,noisy_loss_log_chi2,noisy_loss_log_chi2_separate
from core.utils.helper_functions import reward_variance
import warnings

def log_chi2_final_intensity_spheroid_BO_single_separate(theta,distribution,formfactor,volume,list_q,experiment,uncertainty):
    theta_np = theta.detach().cpu().numpy()
    num_parameters = theta_np.shape[1] - 1 #record number of model parameters
    num_evaluations= list_q.size
    results_chi2 = []
    results_gaussian_residual = [] #for storing normalization term
    if callable(distribution): # check if distribution is mono
        if num_parameters == 5: #sphere case
            for t in theta_np:
                ...
                chi2,residual = noisy_loss_log_chi2_separate(experiment,uncertainty,model_prediction,discrepancy)
                results_chi2.append(chi2)
                results_gaussian_residual.append(residual)
    ...
    return torch.tensor(results_chi2, dtype=torch.double), torch.tensor(results_gaussian_residual, dtype=torch.double)
"""
when bg and porod is removed, use the following nano intensity variant
"""
def log_chi2_nano_intensity_spheroid_BO_single_separate(theta,distribution,formfactor,volume,list_q,experiment,uncertainty):
    theta_np = theta.detach().cpu().numpy()
    num_parameters = theta_np.shape[1]-1 #record number of model parameters
    num_evaluations= list_q.size
    results_chi2 = []
    ...
    return torch.tensor(results_chi2, dtype=torch.double), torch.tensor(results_gaussian_residual, dtype=torch.double)
def log_chi2_final_intensity_spheroid_BO_double_separate(theta, distribution_1, distribution_2, formfactor_1, formfactor_2 ,volume_1, volume_2,list_q,experiment,uncertainty):
    theta_np = theta.detach().cpu().numpy()
    num_parameters = theta_np.shape[1]-1 #record number of columns
    num_evaluations= list_q.size
    results_chi2 = []
    results_gaussian_residual = [] #for storing normalization term
...
def log_chi2_nano_intensity_spheroid_BO_double_separate(theta, distribution_1, distribution_2, formfactor_1, formfactor_2 ,volume_1, volume_2,list_q,experiment,uncertainty):
    theta_np = theta.detach().cpu().numpy()
    num_parameters = theta_np.shape[1]-1 #record number of columns
    num_evaluations= list_q.size
    results_chi2 = []
